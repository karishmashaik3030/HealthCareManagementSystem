...
<tr style="border-bottom:2px solid #000; background-color:#000; color:#fff; font-weight:bold;"><td>Name</td><td>Age</td><td>Sex</td><td>Address</td><td>Problem</td><td>Date</td><td>CardNo.</td><td>Availability</td></tr>
<%
...
st=con.createStatement();
		...
		rs=st.executeQuery("select * from apprequest");
	while(rs.next()){
		...
		for(int val=0;val<details.size();val=val+7){
			%>
			...
			<td><a href="availability.jsp?name=<%=details.get(val).toString() %>&prob=<%=details.get(val+4).toString() %>&date=<%=details.get(val+5).toString() %>">Doctor Availability</a></td>
	<!--		<td><a href="declined.jsp?name=<%=details.get(val).toString() %>&status=parcel_dc">Declined</a></td>
			<td><a href="submitdata.jsp?name=<%=details.get(val).toString() %>&status=parcel_dv">Delivered</a></td>					
			</tr>  -->
				<% 
		}
