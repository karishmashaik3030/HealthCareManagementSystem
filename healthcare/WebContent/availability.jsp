...
<tr style="border-bottom:2px solid #000; background-color:#000; color:#fff; font-weight:bold;"><td>DoctorName</td><td>Id</td><td>Specialist</td><td>Mobile</td><td>Address</td><td>Appointment</td></tr>
<%
...
st=con.createStatement();
		...
		rs=st.executeQuery("select * from docregister where special='"+prob+"'");
	while(rs.next()){
		...
		for(int val=0;val<details.size();val=val+5){
			...
			<td><a href="fixapp.jsp?name=<%=details.get(val).toString() %>&patient=<%=name%>&prob=<%=prob%>&date=<%=date%>">Fix Appointment</a></td>
