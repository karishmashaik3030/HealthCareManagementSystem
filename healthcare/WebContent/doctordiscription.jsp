...
<tr style="border-bottom:2px solid #000; background-color:#000; color:#fff; font-weight:bold;"><td>Doctor Name</td><td>Patient Name</td><td>Problem</td><td>Date</td><td>Priscription</td></tr>
<%

String name=(String)session.getAttribute("name");
String special=(String)session.getAttribute("problem");

ArrayList details=new ArrayList();
Class.forName("com.mysql.jdbc.Driver");
con=DriverManager.getConnection("jdbc:mysql://localhost:3306/healthcare","root","root123");
st=con.createStatement();
System.out.println("Doooocc :"+name);
		rs=st.executeQuery("select * from doctorappointment where name = '"+name+"'");
	while(rs.next()){
		...
		for(int val=0;val<details.size();val=val+4){
			...
			<td><a href="priscription.jsp?name=<%=details.get(val).toString() %>&patient=<%=details.get(val+1).toString()%>&problem=<%=details.get(val+2).toString()%>&date=<%=details.get(val+3).toString()%>">Prescription</a></td>
